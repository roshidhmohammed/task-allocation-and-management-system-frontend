...
const EditTask = ({setShowEditTaskModal, setIsRefetch}) => {
    const editTaskInfo = useSelector((state)=> state.editTask)
     const {
    register,
    handleSubmit,
    reset,
  ...
  if (editTaskInfo) {
    reset({
      ...
      status: editTaskInfo.status || "",
      priority: editTaskInfo.priority || "",
      estimatedHours: editTaskInfo.estimatedHours || null,

      dueDate: editTaskInfo.dueDate
        ? new Date(editTaskInfo.dueDate)
            .toISOString()
            .split("T")[0]
        : "",

      requiredSkills:
        editTaskInfo.requiredSkills || [],
    });
  }
}, [editTaskInfo, reset]);
  ...
  const onSubmit = async (data) => {
    ...
    await axiosInstance
      .put(`/task/${editTaskInfo?._id}`, data, { withCredentials: true })
      .then((res) => {
  ...
  return (
   <FormModal onClick={()=> setShowEditTaskModal(false)} title={"Edit Task"}>
